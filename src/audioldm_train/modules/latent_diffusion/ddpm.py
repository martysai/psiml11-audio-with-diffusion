...
class DDPM(pl.LightningModule):
    ...
    def __init__(
        ...
        # Vendored, locally modified -- see VENDORED.md. Upstream builds a
        # second CLAP instance here (self.clap, needing its own ~2.35GB
        # checkpoint) used only by generate_sample()'s CLAP-similarity
        # candidate re-ranking, which nothing in this project's inference
        # path calls. Dropped to avoid that checkpoint entirely; if you ever
        # add a caller of generate_sample(), you'll need to restore this.
