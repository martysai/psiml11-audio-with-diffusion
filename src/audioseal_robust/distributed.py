...
def seed_everything(seed: int, env: DistEnv) -> None:
    ...
    All four RNG sources this pipeline actually touches are covered:
    `torch.manual_seed` alone seeds only the default CPU generator, not
    CUDA's per-device ones (`torch.cuda.manual_seed_all` covers every
    visible GPU, and is a harmless no-op when there are none); numpy keeps
    its own independent state, which soundfile/librosa/scipy in the
    sgmse/data path can draw from; and `random.seed` covers the Python
    stdlib, which attacks.py's AudioLDMAttack None-strength sampling uses.
    """
    torch.manual_seed(seed + env.rank)
    torch.cuda.manual_seed_all(seed + env.rank)
    np.random.seed(seed + env.rank)
